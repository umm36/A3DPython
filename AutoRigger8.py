...
def spawnJoints(dictArray, side, function):
    ...
    if mc.objExists(rN()+"_jnt_GRP"):                                                                   #if the joint group already exists
        ...
        for allJnts in mc.listRelatives("*_Loc_Arm*", allDescendents = True, type = "locator"):            #for every object that has Loc_Arm in it:
            locListArms.append(mc.pickWalk(allJnts, direction = "Up")[0])                               #add it to the list of arm joints
...
############################################################################################################################################
        ...
        for rig in rigSwitch:                                                                           #for each rig of IK FK or Bind
            for counter, loc in enumerate(locListArms):                                                 #for each thing and location in the locator list
                ...
                if counter + 1 < len(locListArms):                                                      #if the thing isn't the last thing in the list:
