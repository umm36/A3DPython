...
def createArms(side):
    if side == 1:
        if mc.objExists(rN()+"_"+bS(side)+"_Arm_GRP"):
            ...
            "+bS(side)+"_arm = mc.group(em = True, name = rN()+"_"+bS(side)+"_Arm_GRP")
            mc.scale(side,1,1,"+bS(side)+"_arm)
            mc.parent("+bS(side)+"_arm, rN()+"_Loc_SPINE_"+str(mc.intField(spineCount, query = True, value = True) - 1))
            mc.move(12.3*side,148,-1.4,"+bS(side)+"_arm)
            mc.rotate(0,0,0,"+bS(side)+"_arm)
